...
from .models import Assignment, Submission, Grade, Discipline

class AssignmentForm(forms.ModelForm):
    class Meta:
        model = Assignment
        fields = ['title', 'description', 'assignment_type', 'difficulty_level', 
                  'max_score', 'passing_score', 'submission_deadline', 
                  'estimated_duration', 'is_mandatory', 'is_published', 
...
class SubmissionForm(forms.ModelForm):
    class Meta:
        ...
        fields = ['content', 'file']
        widgets = {
            'content': forms.Textarea(attrs={'rows': 6, 'class': 'form-control', 'placeholder': 'Écrivez votre réponse ici...'}),
...
class DisciplineForm(forms.ModelForm):
    class Meta:
        model = Discipline
        fields = ['name', 'code', 'description', 'ects_credits', 'semester', 'is_active']
        widgets = {
